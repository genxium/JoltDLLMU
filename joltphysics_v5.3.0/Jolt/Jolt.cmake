...
if (CMAKE_GENERATOR STREQUAL "Ninja Multi-Config" AND MSVC)
	...
	target_precompile_headers(${TARGET_NAME} PRIVATE "${JOLT_PHYSICS_ROOT}/Jolt.h")
else()
	target_precompile_headers(${TARGET_NAME} PRIVATE "$<$<NOT:$<CONFIG:ReleaseCoverage>>:${JOLT_PHYSICS_ROOT}/Jolt.h>")
endif()

if (NOT CPP_EXCEPTIONS_ENABLED)
	# Disable use of exceptions in MSVC's STL
	target_compile_definitions(${TARGET_NAME} PUBLIC $<$<BOOL:${MSVC}>:_HAS_EXCEPTIONS=0>)
endif()
...
target_compile_definitions(${TARGET_NAME} PUBLIC "$<$<CONFIG:ReleaseASAN,ReleaseTSAN>:JPH_DISABLE_TEMP_ALLOCATOR;JPH_DISABLE_CUSTOM_ALLOCATOR>")

# Setting floating point exceptions
if (FLOATING_POINT_EXCEPTIONS_ENABLED AND "${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
	target_compile_definitions(${TARGET_NAME} PUBLIC "$<$<CONFIG:Debug,Release>:JPH_FLOATING_POINT_EXCEPTIONS_ENABLED>")
endif()
...
if (USE_ASSERTS)
	target_compile_definitions(${TARGET_NAME} PUBLIC JPH_ENABLE_ASSERTS)
endif()

# Setting double precision flag
if (DOUBLE_PRECISION)
	target_compile_definitions(${TARGET_NAME} PUBLIC JPH_DOUBLE_PRECISION)
endif()

# Setting to attempt cross platform determinism
if (CROSS_PLATFORM_DETERMINISTIC)
	target_compile_definitions(${TARGET_NAME} PUBLIC JPH_CROSS_PLATFORM_DETERMINISTIC)
endif()

# Setting to determine number of bits in ObjectLayer
if (OBJECT_LAYER_BITS)
	target_compile_definitions(${TARGET_NAME} PUBLIC JPH_OBJECT_LAYER_BITS=${OBJECT_LAYER_BITS})
endif()

if (USE_STD_VECTOR)
	target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_STD_VECTOR)
endif()
...
if (TRACK_BROADPHASE_STATS)
	target_compile_definitions(${TARGET_NAME} PUBLIC JPH_TRACK_BROADPHASE_STATS)
endif()
...
if (TRACK_NARROWPHASE_STATS)
	target_compile_definitions(${TARGET_NAME} PUBLIC JPH_TRACK_NARROWPHASE_STATS)
endif()

# Enable the debug renderer
if (DEBUG_RENDERER_IN_DISTRIBUTION)
	target_compile_definitions(${TARGET_NAME} PUBLIC "JPH_DEBUG_RENDERER")
elseif (DEBUG_RENDERER_IN_DEBUG_AND_RELEASE)
	target_compile_definitions(${TARGET_NAME} PUBLIC "$<$<CONFIG:Debug,Release,ReleaseASAN,ReleaseUBSAN,ReleaseTSAN>:JPH_DEBUG_RENDERER>")
endif()

# Enable the profiler
if (PROFILER_IN_DISTRIBUTION)
	target_compile_definitions(${TARGET_NAME} PUBLIC "JPH_PROFILE_ENABLED")
elseif (PROFILER_IN_DEBUG_AND_RELEASE)
	target_compile_definitions(${TARGET_NAME} PUBLIC "$<$<CONFIG:Debug,Release,ReleaseASAN,ReleaseUBSAN,ReleaseTSAN>:JPH_PROFILE_ENABLED>")
endif()

# Compile the ObjectStream class and RTTI attribute information
if (ENABLE_OBJECT_STREAM)
	target_compile_definitions(${TARGET_NAME} PUBLIC JPH_OBJECT_STREAM)
endif()
...
function(EMIT_X86_INSTRUCTION_SET_DEFINITIONS)
	if (USE_AVX512)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_AVX512)
	endif()
	if (USE_AVX2)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_AVX2)
	endif()
	if (USE_AVX)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_AVX)
	endif()
	if (USE_SSE4_1)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_SSE4_1)
	endif()
	if (USE_SSE4_2)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_SSE4_2)
	endif()
	if (USE_LZCNT)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_LZCNT)
	endif()
	if (USE_TZCNT)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_TZCNT)
	endif()
	if (USE_F16C)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_F16C)
	endif()
	if (USE_FMADD AND NOT CROSS_PLATFORM_DETERMINISTIC)
		target_compile_definitions(${TARGET_NAME} PUBLIC JPH_USE_FMADD)
	endif()
endfunction()
...
if ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
	...
	elseif (EMSCRIPTEN)
		...
		if (JPH_USE_WASM64)
			target_compile_options(${TARGET_NAME} PUBLIC -sMEMORY64)
			target_link_options(${TARGET_NAME} PUBLIC -sMEMORY64)
		endif()
...
if (EMSCRIPTEN)
	...
	target_link_options(${TARGET_NAME} PUBLIC -Wno-limited-postlink-optimizations)
endif()
