...
void Character::PostSimulationNoLockWithValidation(float inMaxSeparationDistance, ContactListener* contactListener) {
	// Get character position, rotation and velocity
	RVec3 char_pos;
	Quat char_rot;
	Vec3 char_vel;
	auto& biNoLock = sGetBodyLockInterface(mSystem, false);
	const Body* body1 = nullptr;
	{
		BodyLockRead fakeLock(biNoLock, mBodyID);
		if (!fakeLock.Succeeded())
			return;
		const Body &body = fakeLock.GetBody();
		body1 = &body;
		char_pos = body.GetPosition();
		char_rot = body.GetRotation();
		char_vel = body.GetLinearVelocity();
	...
	// Collector that finds the hit with the normal that is the most 'up'
	class MyCollectorWithListner : public CollideShapeCollector
	{
	public:
		// Constructor
		explicit			MyCollectorWithListner(const Body* body1, Vec3Arg inUp, RVec3 inBaseOffset, const BodyLockInterface* biNoLock, ContactListener* contactListener) : mBody1(body1), mBaseOffset(inBaseOffset), mBiNoLock(biNoLock), mUp(inUp), mContactListener(contactListener) { }

		// See: CollectorType::AddHit
		virtual void		AddHit(const CollideShapeResult &inResult) override
		{
			BodyLockRead fakeLock(*mBiNoLock, inResult.mBodyID2);
			if (!fakeLock.Succeeded())
				return;
			const Body &body2 = fakeLock.GetBody();
			
			auto res = mContactListener->OnContactValidate(*mBody1, body2, mBaseOffset /* doesn't matter */, inResult);
	...
	private:
		...
		RVec3				mBaseOffset;
		Vec3				mUp;
		const BodyLockInterface*   mBiNoLock;
		ContactListener*     mContactListener;
		float				mBestDot = -FLT_MAX;
	};

	// Collide shape
	MyCollectorWithListner collector(body1, mUp, char_pos, &biNoLock, contactListener);
	CheckCollision(char_pos, char_rot, char_vel, inMaxSeparationDistance, mShape, char_pos, collector, false);
	...
	// Get additional data from body
	BodyLockRead groundBodyFakeLock(biNoLock, mGroundBodyID);
	if (groundBodyFakeLock.Succeeded())
	{
		const Body &groundBody = groundBodyFakeLock.GetBody();
