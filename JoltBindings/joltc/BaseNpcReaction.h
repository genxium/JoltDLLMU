...
public:
    ...
    */
    ...
    int deriveReactionAgainstGroundAndMvBlocker(int currRdfId, const BodyInterface* biNoLock, const CH_COLLIDER_T* selfNpcCollider, const BodyID& selfNpcBodyID, const uint64_t selfNpcUd, const NpcGoal currNpcGoal, const CharacterDownsync& currChd, const CharacterConfig* cc, CharacterDownsync* nextChd, bool cvSupported, bool cvInAir, bool cvOnWall, bool currNotDashing, bool currEffInAir, bool oldNextNotDashing, bool oldNextEffInAir, bool inJumpStartupOrJustEnded, CharacterBase::EGroundState cvGroundState, bool canJumpWithinInertia, const AABox& visionAABB, const Vec3Arg& visionNarrowPhaseInBaseOffset, const Vec3& visionDirection, const Vec3& visionOverlapAgainstRhs, const BodyID& toHandleMvBlockerBodyID, const uint64_t toHandleMvBlockerUd, const int visionReactionByFar);

    int deriveReactionAgainstMvBlockerAfterApproximation(int currRdfId, const uint64_t selfNpcUd, const CharacterDownsync& currChd, const bool cvSupported, const bool canJumpWithinInertia, const bool isMinGapJumpable, const bool isCurrGapJumpable, const bool currGroundCanHoldMeIfWalkOn, const bool currGapCanHoldMeIfWalkOn, const float currDxAbsToGroundBodyEdge, const bool temptingToMove, const int visionReactionByFar);

    bool isGapJumpable(const float toJumpDxAbs, const float toJumpDy, const float avgVelXAbs, const float chJumpInitVelY);
};
