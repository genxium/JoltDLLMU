...
protected:
    ...
    Body*             getOrCreateCachedBulletCollider(const uint64_t ud, const BulletConfig* bc, float newHalfBoxSizeX, float newHalfBoxSizeY);
    ...
    inline const CharacterDownsync& immutableCurrChdFromUd(uint64_t udt, uint64_t payload) {
        return (UDT_PLAYER == udt ? transientUdToCurrPlayer[payload]->chd() : transientUdToCurrNpc[payload]->chd());
    }

    inline CharacterDownsync* mutableNextChdFromUd(uint64_t udt, uint64_t payload) {
        return (UDT_PLAYER == udt ? transientUdToNextPlayer[payload]->mutable_chd() : transientUdToNextNpc[payload]->mutable_chd());
    }
