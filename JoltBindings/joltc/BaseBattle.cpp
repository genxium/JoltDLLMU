...
RenderFrame* BaseBattle::SingleStep(int currRdfId, int delayedIfdId, InputFrameDownsync* delayedIfd) {
...
*/
        ...
        bool cvOnWall = (!groundBodyID.IsInvalid() && !groundBodyIsChCollider && single->IsSlopeTooSteep(single->GetGroundNormal()));
...
bool BaseBattle::WriteSingleStepFrameLog(int currRdfId, RenderFrame* nextRdf, int fromRdfId, int toRdfId, int delayedIfdId, InputFrameDownsync* delayedIfd) {
    ...
    nextFrameLog->set_timer_rdf_id(0);
...
bool BaseBattle::Step(int fromRdfId, int toRdfId, DownsyncSnapshot* virtualIfds) {
    for (int currRdfId = fromRdfId; currRdfId < toRdfId; currRdfId++) {
        ...
        RenderFrame* nextRdf = SingleStep(currRdfId, delayedIfdId, delayedIfd);
        if (frameLogEnabled) {
            WriteSingleStepFrameLog(currRdfId, nextRdf, fromRdfId, toRdfId, delayedIfdId, delayedIfd);
        }
    }
    return true;
}
