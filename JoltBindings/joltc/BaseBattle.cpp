...
RenderFrame* BaseBattle::CalcSingleStep(const int currRdfId, int delayedIfdId, InputFrameDownsync* delayedIfd) {
    ...
    for (int i = 0; i < currRdf->trigger_count(); i++) {
        ...
        uint32_t triggerId = currTrigger.id();
        if (globalPrimitiveConsts->terminating_trigger_id() == triggerId) break;
        ...
        // [WARNING] Recover "magic_quota_infinite" after "stepOtherSingleTriggerState", NOT BEFORE.
        if (0 >= currTrigger.quota()) {
            ...
            if (triggerConfigFromTileDict.count(steppingTriggerId)) {
                ...
                if (globalPrimitiveConsts->magic_quota_infinite() == triggerConfigFromTiled->quota()) {
                    nextTrigger->set_quota(globalPrimitiveConsts->magic_quota_infinite());
                }
            }
        }
    }
    ...
    for (int i = 0; i < currRdf->npc_count(); i++) {
        ...
        auto handle = jobSys->CreateJob("npc-pre-physics-update", JPH::Color::sBlack, [currRdfId, i, currRdf, nextRdf, this, dt]() {
            ...
            bool gravityDirty = false, frictionDirty = false;
            const CharacterConfig* cc = getCc(currChd.species_id());
            if (!noOpSet.count(currChd.ch_state()) || Awaking == currChd.ch_state()) {
                const CharacterBattleSpecificConfig* chOverride = getChOverride(ud);
                auto currChState = currChd.ch_state();
                bool currNotDashing = BaseBattleCollisionFilter::chIsNotDashing(currChd);
                bool currDashing = !currNotDashing;
            ...
            } else if (Dimmed == currChd.ch_state()) {
                const uint64_t subscribingToTriggerUd = calcTriggerUserData(currNpc.subscribes_to_trigger_id());;
                const Trigger* currSubscribingToTrigger = transientUdToCurrTrigger.at(subscribingToTriggerUd);
                const Trigger* nextSubscribingToTrigger = transientUdToNextTrigger.at(subscribingToTriggerUd);
                const bool subscribingToTriggerMainCycleTicked = trivialTrtSet.count(currSubscribingToTrigger->trt()) ? (nextSubscribingToTrigger->quota() < currSubscribingToTrigger->quota()) : false;

                if (subscribingToTriggerMainCycleTicked) {
                    nextNpc->set_activated_rdf_id(currRdfId);
                    nextChd->set_ch_state(Awaking);
                    nextChd->set_frames_in_ch_state(0);
                    nextChd->set_frames_invinsible(0 >= cc->awaking_frames_invinsible() ? globalPrimitiveConsts->default_frames_delayed_of_boss_savepoint() : cc->awaking_frames_invinsible());
                    nextChd->set_frames_to_recover(0 >= cc->awaking_frames_to_recover() ? globalPrimitiveConsts->default_frames_delayed_of_boss_savepoint() : cc->awaking_frames_to_recover());
                }
            }
                ...
                if (!noOpSet.count(nextChd->ch_state()) || BlownUp1 == nextChd->ch_state()) {
                    ...
                    if (cc->has_vision_reaction() && notTurningAround) {
                        ...
                        if (nullptr != npcReaction) {
                            ...
                            Vec3 mvIntentionNorm = antiGravityNorm;
                            if (currIsFlying) {
                                if (!inputInducedMotion->velCOM.IsNearZero()) {
                                    mvIntentionNorm = inputInducedMotion->velCOM.Normalized();
                                } else {
...
bool BaseBattle::ResetStartRdf(WsReq* initializerMapData) {
    if (nullptr != initializerMapData->GetArena() && initializerMapData->GetArena() != &pbTempAllocator) {
        WsReq* copyWriter = google::protobuf::Arena::Create<WsReq>(&pbTempAllocator);
        copyWriter->CopyFrom(*initializerMapData);
        initializerMapData = copyWriter; 
    }
    /* [WARNING] 
    ...
    */
    ...
    for (int i = 0; i < startRdf->dynamic_traps_size(); i++) {
        ...
        if (!c->init_not_moving()) {
            ...
            if (globalPrimitiveConsts->tpts().sliding_platform() == tp->tpt() || globalPrimitiveConsts->tpts().rotating_platform() == tp->tpt()) {
...
bool BaseBattle::initTriggerMainAndSubCycles(RenderFrame* startRdf) {
    ...
    for (int i = 0; i < startRdf->npcs_size(); ++i) {
        ...
        if (!collectedPublishingMaskUponExhausted.count(targetTriggerId)) {
            ...
            if (globalPrimitiveConsts->trts().flip_flop() == targetTriggerConfigFromTile->trt()) {
                collectedPublishingMaskUponExhausted[targetTriggerId] = 1;
            } else {
                collectedPublishingMaskUponExhausted[targetTriggerId] <<= 1;
            }
    ...
    for (int i = 0; i < startRdf->triggers_size(); ++i) {
        ...
        if (globalPrimitiveConsts->trts().flip_flop() == tr->trt()) {
            tr->set_sub_cycle_mask_to_fulfill(1UL);
            continue;
        }
...
void BaseBattle::processInertiaWalkingHandleZeroEffDx(const int currRdfId, float dt, const CharacterDownsync& currChd, const MassProperties& massProps, const Vec3& currChdFacing, CharacterDownsync* nextChd, int effDy, const CharacterConfig* cc, const CharacterBattleSpecificConfig* chOverride, bool effInAir, bool currParalyzed, const bool isInWalkingAtkAndNotRecovered, const uint64_t ud, const CH_COLLIDER_T* chCollider, const bool currDashing, InputInducedMotion* ioInputInducedMotion, bool& ioGravityDirty, bool& ioFrictionDirty) {
    ...
    if (isCrouching(currChd.ch_state(), cc)) {
        ...
        if (isOnRegularSlopeOrPStairs && 0 != currChd.ground_norm_x()) {
...
*/
    ...
    if (0 != effDx && hasNonZeroSpeed) {
        ...
        if (0 == forceX) {
            ...
            bool isOnStairsP;
            bool isOnRegularSlope;
            bool walkingUpSlope;
            bool walkingDownSlope;
            Vec3 slopeOutwardNorm;
            checkWalkingOnSlope(currChd, forceX, isOnStairsP, isOnRegularSlope, walkingDownSlope, walkingUpSlope, slopeOutwardNorm);
            if (walkingUpSlope || walkingDownSlope) {
                ...
                if (!origVelRelativeToGround.IsNearZero()) {
                    Vec3 newVelRelativeToGround = origVelRelativeToGround;
                    calcResidueWrtGroundNormal(currChdGroundNorm, newVelRelativeToGround);
                ...
                if (!origForce.IsNearZero()) {
                    Vec3 newForce = origForce;
                    calcResidueWrtGroundNormal(currChdGroundNorm, newForce);
...
void BaseBattle::processInertiaFlyingHandleZeroEffDxAndDy(const int currRdfId, float dt, const CharacterDownsync& currChd, const MassProperties& massProps, const Vec3& currChdFacing, CharacterDownsync* nextChd, const CharacterConfig* cc, const CharacterBattleSpecificConfig* chOverride, bool currParalyzed, const bool isInWalkingAtkAndNotRecovered, const uint64_t ud, const CH_COLLIDER_T* chCollider, const bool currDashing, InputInducedMotion* ioInputInducedMotion, bool& ioGravityDirty, bool& ioFrictionDirty) {
    if (!currDashing) {
        ...
        if (IsLengthNearZero(newVelX * dt) && IsLengthNearZero(newVelY * dt)) {
            newVelX = 0;
            newVelY = 0;
...
InputFrameDownsync* BaseBattle::getOrPrefabInputFrameDownsync(int inIfdId, uint32_t inSingleJoinIndex, uint64_t inSingleInput, bool fromUdp, bool fromTcp, bool& outExistingInputMutated) {
    ...
    while (ifdBuffer.EdFrameId <= inIfdId) {
        ...
        ifdHolder->set_input_count(playersCnt);
        for (int k = 0; k < playersCnt; k++) {
            uint64_t prefabbedInput = prefabbedInputList[k];
            if (k < ifdHolder->input_list_size()) {
                ifdHolder->set_input_list(k, prefabbedInput);
            } else {
    ...
    InputFrameDownsync* ret = ifdBuffer.GetByFrameId(inIfdId);
    if (fromTcp) {
    ...
    for (int i = 0; i < currRdf->dynamic_trap_count(); i++) {
        ...
        if (globalPrimitiveConsts->tpts().boss_door() == tpt) {
            ...
            if (shouldFlip) {
                ...
                std::ostringstream oss;
#endif
                switch (currTp.trap_state()) {
                case TpIdle:
                case TpActivated:
                    nextTp->set_trap_state(TrapState::TpDeactivated);
                    nextTp->set_frames_in_trap_state(0);
#ifndef NDEBUG
                    oss << "@currRdfId=" << currRdfId << ", BossDoor ud=" << ud << " flipped Idle->Deactivated by trigger id=" << tpConfigFromTile->subscribes_to_trigger_id() << std::endl;
                    Debug::Log(oss.str(), DColor::Orange);
#endif
                    break;
                case TpDeactivated:
                    nextTp->set_trap_state(TrapState::TpActivated);
                    nextTp->set_frames_in_trap_state(0);
#ifndef NDEBUG
                    oss << "@currRdfId=" << currRdfId << ", BossDoor ud=" << ud << " flipped Deactivated->Idle by trigger id=" << tpConfigFromTile->subscribes_to_trigger_id() << std::endl;
                    Debug::Log(oss.str(), DColor::Orange);
#endif
                    break;
                }
...
*/
        ...
        if (!currIsFlying) {
            ...
            if (!currParalyzed && globalPrimitiveConsts->pattern_id_unable_to_op() != patternId && 0 == currChd.locking_on_ud() && cc->anti_gravity_when_idle() && 0 >= currChd.frames_to_recover() && InAirIdle1NoJump != currChd.ch_state() && cc->anti_gravity_frames_lingering() < currChd.frames_in_ch_state()) {
#ifndef NDEBUG
                std::ostringstream oss;
                oss << "@currRdfId=" << currRdfId << ", flying characterUd=" << ud << " turning anit-gravity idle: currChS=" << currChd.ch_state() << ", currFc=" << currChd.frames_in_ch_state();
                Debug::Log(oss.str(), DColor::Orange);
#endif
                nextChd->set_ch_state(InAirIdle1NoJump);
...
void BaseBattle::ClearChd(CharacterDownsync* chd) {
    ...
    chd->set_aiming_q_w(1);

    chd->set_walkstopping_rdf_countdown(0);
    chd->set_fallstopping_rdf_countdown(0);
    ...
    chd->set_frames_invinsible(0);

    chd->set_bullet_team_id(globalPrimitiveConsts->terminating_bullet_team_id());
    chd->set_remaining_air_jump_quota(0);
    chd->set_remaining_air_dash_quota(0);

    chd->set_damaged_hint_rdf_countdown(0);
    chd->set_damaged_elemental_attrs(0);

    chd->set_remaining_def1_quota(0);

    chd->set_combo_hit_cnt(0);
    chd->set_combo_frames_remained(0);

    chd->set_last_damaged_by_ud(0);
    chd->set_last_damaged_by_bullet_team_id(globalPrimitiveConsts->terminating_bullet_team_id());
    
    chd->set_active_skill_id(globalPrimitiveConsts->no_skill());
    chd->set_active_skill_hit(globalPrimitiveConsts->no_skill_hit());

    chd->set_btn_a_holding_rdf_cnt(0);
    chd->set_btn_b_holding_rdf_cnt(0);
    ...
    chd->set_flying_rdf_countdown(0);

    chd->set_locking_on_ud(0);

    chd->set_ch_collision_team_id(globalPrimitiveConsts->terminating_bullet_team_id());
    chd->set_speed(0);

    chd->set_hp(0);
    chd->set_mp(0);

    chd->set_ch_state(CharacterState::Idle1);
    chd->set_frames_in_ch_state(0);   
    chd->set_lower_part_rdf_cnt(0);
}
...
void BaseBattle::CopyIfd(const InputFrameDownsync* from, InputFrameDownsync* to) {
    ...
    to->set_input_count(from->input_count());
    for (int k = 0; k < from->input_list_size(); k++) {
        if (k < to->input_list_size()) {
            to->set_input_list(k, from->input_list(k));
        } else {
...
void BaseBattle::CopyChd(const CharacterDownsync* from, CharacterDownsync* to) {
    ...
    // Buff list
    to->set_buff_count(from->buff_count());
    for (int i = 0; i < from->buff_list_size(); i++) {
        const Buff* fromSingle = &(from->buff_list(i));
        Buff* toSingle = i < to->buff_list_size() ? to->mutable_buff_list(i) : to->add_buff_list();
    ...
    // Debuff list
    to->set_debuff_count(from->debuff_count());
    for (int i = 0; i < from->debuff_list_size(); i++) {
        const Debuff* fromSingle = &(from->debuff_list(i));
        Debuff* toSingle = i < to->debuff_list_size() ? to->mutable_debuff_list(i) : to->add_debuff_list();
        toSingle->CopyFrom(*fromSingle); // All primitive types, no memory alloc/free would happen
    }
    ...
    */
    ...
    for (int i = 0; i < from->inventory_slots_size(); i++) {
        ...
        InventorySlot* toSingle = i < to->inventory_slots_size() ? to->mutable_inventory_slots(i) : to->add_inventory_slots();
        toSingle->CopyFrom(*fromSingle); // All primitive types, no memory alloc/free would happen
    }
    ...
    // Kinematic knobs
    to->set_kk_count(from->kk_count());
    for (int i = 0; i < from->kinematic_knobs_size(); i++) {
        if (i < to->kinematic_knobs_size()) {
            to->set_kinematic_knobs(i, from->kinematic_knobs(i));
        } else {
...
void BaseBattle::stepSingleChdState(const int currRdfId, const RenderFrame* currRdf, RenderFrame* nextRdf, const float dt, const uint64_t ud, const uint64_t udt, const CharacterConfig* cc, const CharacterBattleSpecificConfig* chOverride, CH_COLLIDER_T* single, const CharacterDownsync& currChd, const bool currIsFlying, CharacterDownsync* nextChd, bool& groundBodyIsChCollider, bool& isDead, bool& cvOnWall, bool& cvSupported, bool& cvInAir, bool& inJumpStartupOrJustEnded, CharacterBase::EGroundState& cvGroundState, InputInducedMotion* inputInducedMotion, uint64_t& outClosestOffenderUd, int& outClosestOffenderBulletTeamId, float& outClosestOffenderScore, Vec3& outClosestOffenderPosDiff) {
    ...
    */
    ...
    bool walkingDownSlope;
    Vec3 slopeOutwardNorm;
    checkWalkingOnSlope(currChd, inputInducedMotion->forceCOM.GetX(), isOnStairsP, isOnRegularSlope, walkingDownSlope, walkingUpSlope, slopeOutwardNorm);
    ...
    if (cvOnWall || currIsFlying) {
        ...
        if (!cvSupported) {
            if ((walkingUpSlope || walkingDownSlope) && isFreeForGroundReattachment(nextChd->ch_state())) {
                // Possibly unintended but rushed off the edge of a p-type stairs or a regular slope
                ...
                if (!newGroundBodyID.IsInvalid()) {
                    newGroundNormal = slopeOutwardNorm;
                    single->SetGroundState(JPH::CharacterBase::EGroundState::OnGround);
                    cvSupported = true;
                }
            }
    ...
    if (cvSupported) {
        ...
        const bool roughlyOnGroundTop = isLhsCharacterRoughlyOnRhsTop(currChd, newGroundAABB);
        newGroundVel = biNoLock->GetLinearVelocity(newGroundBodyID);
        rectifyGroundNormal(newGroundUd, isOnStairsP, isOnRegularSlope, roughlyOnGroundTop, slopeOutwardNorm, newGroundNormal);

/*
#ifndef NDEBUG
        if (Walking == currChd.ch_state() && 120 <= currChd.frames_in_ch_state() && isOnStairsP) {
            std::ostringstream oss;
            oss << "@currRdfId=" << currRdfId << ", character ud=" << ud << ", pos=(" << currChd.x() << "," << currChd.y() << ") might be stuck in p-stairs ud=" << newGroundUd << ", walkingDownSlope=" << walkingDownSlope << ", walkingUpSlope=" << walkingUpSlope << ", roughlyOnGroundTop=" << roughlyOnGroundTop << ", slopeOutwardNorm=(" << slopeOutwardNorm.GetX() << "," << slopeOutwardNorm.GetY() << ")" << ", newGroundNormal=(" << newGroundNormal.GetX() << "," << newGroundNormal.GetY() << "), beforeRectificationGroundNormal=(" << collector.mGroundNormal.GetX() << "," << collector.mGroundNormal.GetY() << ")" << std::endl;
            Debug::Log(oss.str(), DColor::Orange);
        }
#endif
*/
...
#ifndef NDEBUG
            std::ostringstream oss;
            oss << "@currRdfId=" << currRdfId << ", character ud=" << ud << ", bodyId=" << single->GetBodyID().GetIndexAndSequenceNumber() << " forced to crouch#1 from oldNextChState=" << oldNextChState << std::endl;
            Debug::Log(oss.str(), DColor::Orange);
#endif
*/
        }
    } else {
        if (cvSupported && collector.mCrouchForced) {
/*
#ifndef NDEBUG
            std::ostringstream oss;
            oss << "@currRdfId=" << currRdfId << ", character ud=" << ud << ", bodyId=" << single->GetBodyID().GetIndexAndSequenceNumber() << " forced to crouch#2 from oldNextChState=" << oldNextChState << std::endl;
            Debug::Log(oss.str(), DColor::Orange);
#endif
*/
            inputInducedMotion->crouchForcedWhileSupported = true;
...
void BaseBattle::stepOtherSingleTriggerState(const int currRdfId, const Trigger& currTrigger, Trigger* nextTrigger, RenderFrame* nextRdf, StepResult* stepResult) {
    ...
    if (mainCycleCooledDown) {
        ...
        if (globalPrimitiveConsts->trts().flip_flop() == currTrigger.trt()) {
            nextTrigger->set_main_cycle_mask_to_fulfill(1);
            nextTrigger->set_sub_cycle_mask_to_fulfill(1);
        }
#ifndef NDEBUG
        std::ostringstream oss;
    ...
    if (mainCycleExhausted) {
        if (nullptr != triggerConfigFromTiled) {
            uint32_t receivingTriggerId = triggerConfigFromTiled->publishing_to_trigger_id_upon_exhausted();
            if (globalPrimitiveConsts->terminating_trigger_id() != receivingTriggerId) {
                uint64_t nextReceivingTriggerUd = calcTriggerUserData(receivingTriggerId);
                Trigger* nextReceivingTrigger = transientUdToNextTrigger.at(nextReceivingTriggerUd);
                uint64_t publishingMask = triggerConfigFromTiled->publishing_mask_upon_exhausted();
                publishToTrigger(currRdfId, publishingMask, currTrigger.offender_ud(), currTrigger.offender_bullet_team_id(), nextReceivingTrigger);
#ifndef NDEBUG
                uint64_t nextReceivingTriggerMainCycleMaskNow = nextReceivingTrigger->main_cycle_mask_to_fulfill();
                uint64_t nextReceivingTriggerSubCycleMaskNow = nextReceivingTrigger->sub_cycle_mask_to_fulfill();
                int nextReceivingTriggerQuotaNow = nextReceivingTrigger->quota();
                std::ostringstream oss;
                oss << "@currRdfId=" << currRdfId << ", steppingTriggerId=" << steppingTriggerId << " main-cycle exhausted, published mask=" << publishingMask << " to receivingTriggerId=" << receivingTriggerId << ", updated receivingTrigger={main_cycle_mask_to_fulfill: " << nextReceivingTriggerMainCycleMaskNow << ", sub_cycle_mask_to_fulfill:" << nextReceivingTriggerSubCycleMaskNow << ", quota:" << nextReceivingTriggerQuotaNow << "}";
                Debug::Log(oss.str(), DColor::Orange);
#endif
            }
        }
...
void BaseBattle::handleLhsCharacterCollisionWithRhsBullet(
    ...
    if (rhsBlConfig->remains_upon_hit()) {
        ...
        while (immuneRcdI < nextChd->bir_count()) {
...
*/
    ...
    Vec3Arg blInitPushbackVelocity(
        (successfulDef1 ? cc->def1_damage_yield() : 1.0f)*rhsBlConfig->pushback_vel_x(), 
        (successfulDef1 ? (currIsFlying ? cc->def1_damage_yield() : 0.0f) : 1.0f)*rhsBlConfig->pushback_vel_y(), 
        0
    );
