...
void* APP_CreateBattle(char* inBytes, int inBytesCnt, bool isFrontend, bool isOnlineArenaMode, int inSelfJoinIndex) {
    BaseBattle* result = nullptr;
    if (isFrontend) {
        result = new FrontendBattle(inBytes, inBytesCnt, 512, globalPrimitiveConsts->default_backend_input_buffer_size(), globalTempAllocator, isOnlineArenaMode, inSelfJoinIndex);
    } else {
...
bool BACKEND_ProduceDownsyncSnapshot(void* inBattle, uint64_t unconfirmedMask, int stIfdId, int edIfdId, char* outBytesPreallocatedStart, long* outBytesCntLimit, int withRefRdfId) {
    auto backendBattle = static_cast<BackendBattle*>(inBattle);
    return backendBattle->ProduceDownsyncSnapshotAndSerialize(unconfirmedMask, stIfdId, edIfdId, outBytesPreallocatedStart, outBytesCntLimit, withRefRdfId);
}
